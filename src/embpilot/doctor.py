...
def _check_serial_ports() -> CheckResult:
    try:
        ...
        return CheckResult(
            "serial ports", True, ", ".join(ports) if ports else "(none detected)"
        )
    except Exception as exc:  # noqa: BLE001
...
def run_doctor() -> int:
    """Run all checks, print a report, return 0 if required checks pass else 1."""
    ...
    for name, path, vattr in CORE_DEPS:
        r = _check_import(name, path, vattr)
        _print(r)
        required.append(r)
    ...
    print("\n== drivers ==")
    for drv in DRIVERS:
        r = _check_import(drv, drv, None)
        _print(r)
        required.append(r)

    print("\n== storage ==")
    sqlite_r = CheckResult("sqlite3", True, f"v{sqlite3.sqlite_version}")
    _print(sqlite_r)
    data_r = _check_data_dir()
    _print(data_r)
    required.append(data_r)
